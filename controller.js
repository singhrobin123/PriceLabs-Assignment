...
module.exports = async function(address, pageSize) {
    try {
      ...
      const responseData = await fetchData(JSON.stringify(data), 'https://www.vrbo.com/serp/g');
      const listings = searchObjectForKey(responseData, 'listings');
      const csvData = [];
      const maxEndDate = new Date(Math.max(...listings.map(listing => Date.parse(listing.rateSummary.endDate)))).toISOString().slice(0, 10);
      ...
      generateCsv(csvData, maxEndDate);
